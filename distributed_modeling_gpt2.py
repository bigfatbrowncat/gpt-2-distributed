...
# Modified by Ilya Mizus (bigfatbrowncat) in 2022
# This file is an improved version of modeling_gpt2.py from transformers package version 4.25.1
# Let's make sure that the package version on your environment is the same, or this file won't fit!
import pkg_resources
transformers_version = pkg_resources.get_distribution("transformers").version
supported_transformers_versions = [ '4.25.1' ]
if transformers_version not in supported_transformers_versions:
    raise RuntimeError(f"The transformers version {transformers_version} is not suported!")
    ...
    def forward(
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if self.model_parallel:
                torch__cuda__set_device(hidden_states.device)
...
def torch__cuda__set_device(device: torch.cuda._device_t) -> None:
    ...
    """
    device = torch.cuda._utils._get_device_index(device, allow_cpu=True)
    if device >= 0:
...
class GPT2LMHeadModel(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if self.model_parallel:
            torch__cuda__set_device(self.transformer.first_device)
